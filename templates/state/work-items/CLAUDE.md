...
- Never move work item directories between queues by hand. Call `bun ../scripts/move.ts <id> <target-queue>` (run from `state/`) so the move and any related bookkeeping (e.g. updating `current-state.md`) stay consistent.
