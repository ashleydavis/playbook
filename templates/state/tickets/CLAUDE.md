...
- Each ticket is a directory named by its ID, containing `index.md` plus an optional `evidence/` subdir. New tickets always start in `todo/`. The directory name must equal the `**ID:**` field declared in `index.md`.
