...
1. **Relax the cap in [scripts/next-items.ts](scripts/next-items.ts).** The artificial `todo + in-progress <= LIMIT` trim ([next-items.ts:84-94](scripts/next-items.ts#L84-L94)) exists only because the old loop hand-managed concurrency. The workflow pool now manages in-flight count, so `nextItems()` must report **every** dependency-ready `todo` item, not a capped slice.
   ...
   - Rewrite the top-of-file docstring: the cap is no longer applied here; `todo` is now "every actionable item, ordered by ID"; concurrency is bounded by the workflow pool.

2. **Create the workflow script [.claude/workflows/pb-next.js](.claude/workflows/pb-next.js).** A self-contained JS workflow (begins with the required `export const meta = {...}` literal). It receives the queue report via `args` (the parent runs `next-items.ts` and passes the JSON) and returns a structured outcome the parent uses to update `current-state.md`. Structure:
   - `meta`: `name: 'pb-next'`, `description`, `phases: [{title:'Merge'},{title:'Implement'},{title:'Review'}]`.
   - **`args` shape**: `{ mergeQueue: string[], poolItems: string[], agentReview: string[], poolSize: number }`. `poolItems` is `in-progress` items plus dependency-ready `todo` items (the parent concatenates them).
   - **Merge phase** (`phase('Merge')`): `parallel()` over `mergeQueue`, one `agent()` per id running the existing merge goal text from [next.md:53-55](.claude/commands/pb/next.md#L53-L55). Each agent calls `finalize-work-item.ts` and runs post-merge checks itself. This is a genuine barrier: merges can land dependencies into `done/`, and the broken-main rule means a post-merge failure must stop the run before any implement work starts. The phase returns per-item `{id, status: 'merged'|'conflict-failed'|'broken-main'|'timeout'}` via a `StructuredOutput` schema.
   - **Implement/Review pool** (`phase('Implement')` / `phase('Review')`): a hand-written concurrency-limited pool (plain JS) of size `args.poolSize` that pulls ids from `poolItems` and runs each through two sequential `agent()` stages — implement then agent-review — refilling the slot the instant an item finishes both stages or fails out. This is the core change: not `parallel()` (barrier) and not a fixed-list `pipeline()` (drains then stops), but a worker loop that keeps `poolSize` items active until the work-list is exhausted.
     - Implement stage: the implement goal text from [next.md:67-68](.claude/commands/pb/next.md#L67-L68), with the Debug-item exception ([next.md:73](.claude/commands/pb/next.md#L73)).
     ...
     - A stage that fails (agent records it via `fail-work-item.ts` and moves the item back to `todo/` or `blocked/`) drops the item from the pool and frees its slot; the pool continues. Each item returns `{id, stage, status, evidenceDir}`.
   - **`agentReview`-only items**: ids already in `agent-review/` at run start (from an interrupted run) enter the pool at the review stage only.
   - **Return value**: `{ merges: [...], items: [...] }` — every per-item outcome, so the parent can reflect moves and failures into `current-state.md` and detect a systemic failure (two or more items failing the same stage).
...
3. **Rewrite the Steps section of [.claude/commands/pb/next.md](.claude/commands/pb/next.md).** Replace the per-turn `/goal`-loop description ([next.md:41-89](.claude/commands/pb/next.md#L41-L89)) with the workflow-driven outer loop. The parent now:
   1. Runs `bun ../scripts/next-items.ts` from `state/` (Bash) to get the report. (The orientation rule at [next.md:24](.claude/commands/pb/next.md#L24) — no filesystem exploration before the report — stays.)
   ...
   3. On return, inspects the merge outcomes: if any is `broken-main`, records it, leaves the item in `todo/`, updates `current-state.md`, and **stops** (broken-main exception, [next.md:39](.claude/commands/pb/next.md#L39)).
   4. Re-runs `next-items.ts` and updates `current-state.md` for every item that moved or was created (Fix items spawned from proven Debug items included).
   5. Repeats the cycle while the fresh report still lists actionable work (retried items that returned to `todo/` under the failure cap reappear here) and no systemic failure was seen and the turn budget is not exhausted.
   ...
   - Update the "## Example" block ([next.md:91-102](.claude/commands/pb/next.md#L91-L102)) to show continuous replacement (a slow item not blocking new admissions).
...
4. **Update the reference docs to match the new mechanism.**
   ...
   - [handbook.md:306](handbook.md#L306): the `/pb:next` description ("each turn processes ... picks up to 10 unblocked todo items ... runs a per-item sub-agent through each stage") → describe the workflow-driven continuous pool that keeps 10 in flight and refills as items finish.
   - [process.md:111](process.md#L111): the table row "Pick up to 10 unblocked items, implement in parallel" → "Keep up to 10 items in flight, refilling as each completes".
   - [process.md:120](process.md#L120): the `/goal` description still says `pb:next` uses a top-level loop goal and per-item sub-agent goals — update to reflect that orchestration is now a Workflow script and the per-item goals are the workflow's `agent()` stage goals; the deterministic loop replaces the model-driven `/goal` loop.
   - Leave the failure-handling and evidence sections in both docs unchanged (the rules are unchanged).

5. **Update [scripts/CLAUDE.md](scripts/CLAUDE.md)** `next-items.ts` bullet to state it now reports every actionable item (no concurrency cap applied), since the pool is enforced by the workflow.

## Unit Tests

Update [scripts/next-items.test.ts](scripts/next-items.test.ts):
- Replace any test asserting the `todo + in-progress <= LIMIT` trim with a test asserting **all** dependency-ready `todo` items are returned regardless of `in-progress` count (e.g. 15 ready todo items + 3 in-progress → all 15 reported in `todo`).
- Keep/confirm: dependency-readiness filtering (an item with an unmerged dependency is excluded; an item whose deps are all in `done/` is included).
- Keep/confirm: empty queues return `[]`; items sorted by ID.
...
- Build a throwaway state tree with >10 ready `todo` items and a few `in-progress`, run `next-items.ts`, and assert the JSON `todo` array contains every ready id (no cap), `in-progress` is the full list, and an item with an unmerged dependency is absent.
...
- From [scripts/](scripts/): `bun run test` — all unit tests pass, including the updated `next-items` tests.
- From [scripts/](scripts/): `bun run smoke` — all smoke tests pass, including the new uncapped-report check.
- `bun ../scripts/next-items.ts` runs clean from a `state/` fixture and emits valid JSON with the four expected keys.
...
- **Why a worker pool, not `pipeline()`:** `pipeline()` drains a fixed item list and stops; `parallel()` is a barrier. The user's requirement is "keep 10 in flight; when one finishes, start another." That is a concurrency-limited pool that pulls from a work-list and refills on completion — a few lines of plain JS in the workflow body, fully within the documented script API.
...
- **Dependency dynamism within the implement pool is nil:** forward-flow items stop at `human-review/` and never reach `done/` during a run, so no new dependencies are satisfied mid-pool. Readiness only changes across the merge boundary, which the parent handles by re-running `next-items.ts` between workflow invocations.
