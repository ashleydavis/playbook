...
1. **Create the shared commit helper `scripts/commit-state.ts`.**
   - Export `type GitRunner = (cwd: string, args: string[]) => Promise<{ code: number; stdout: string; stderr: string }>` and a default `realGit` runner using `Bun.spawn(["git", "-C", cwd, ...args])` (mirror the runner already in `finalize-work-item.ts`).
...
7. **Update `scripts/CLAUDE.md`.** Add `commit-state.ts <message> [pathspec...]` to the helper list with a one-line description, and add a sentence stating that `move`, `setup-work-item`, `fail-work-item`, and `reset-failures` now commit their own state change (item-scoped, lock-safe) via `commit-state.ts`, while `finalize-work-item` does not (the follow-up `move` commits that transition).

8. **Add the audit-log rule to `process.md`.** In the **Queues** section (near the `move.ts` / `current-state.md` bullets), add a short paragraph: the state repo is a git repo and every significant change is committed so its history is an audit log. State it as: the mutation scripts commit automatically; for hand edits (a `current-state.md` update, a newly created work item) the agent commits with `bun ../scripts/commit-state.ts "<message>" <pathspec>` immediately after; and because script commits are item-scoped, evidence and History notes written before a `move.ts` are captured by that move's commit (so they need no separate commit). Keep it tight (the file is the AI-facing concise reference).
...
- No new unit tests are needed for `move`/`setup-work-item`/`fail-work-item`/`reset-failures` core functions — their commit happens in the CLI `main()` wrapper, which the existing unit tests deliberately do not exercise. Add a brief comment in each modified `main()` pointing to the smoke test that covers the commit.
...
- `grep` the modified skills and docs to confirm: every item-creating skill calls `commit-state.ts`; `next.md` commits `current-state.md` via the parent only; both bootstrap skills `git init` the state repo. Confirm `finalize-work-item.ts` was *not* given a state commit.

## Human Verification

- Run a real `pb:next` turn against a project whose state repo has been `git init`ed, then `git -C state log --oneline --stat` and confirm: each item's stage transitions appear as separate item-scoped commits, the per-turn `current-state.md` update is its own commit, and no commit mixes two different items' directories.
...
- **Concurrency:** `pb:next` runs up to 10 sub-agents in parallel against the single shared state-repo index. Item-scoped pathspecs keep each commit's contents clean; the lock-retry loop in `commit-state.ts` is what makes concurrent `git add`/`commit` safe against `index.lock` contention. This preserves the existing rule that only the parent writes/commits `current-state.md`.
...
- **Open question:** whether to also commit large evidence artefacts (screenshots, transcripts) — current plan commits them as part of the item-scoped move commit, which is the simplest and keeps the audit log complete. Revisit only if state-repo size becomes a problem.
