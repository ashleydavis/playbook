...
- If a project's project and state repos already exist, start with `/pb:status`: read `state/current-state.md`, summarise what is in flight and blocked, and recommend the next skill (`/pb:next`, `/pb:review`, `/pb:plan:break`, ...).
