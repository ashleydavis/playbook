#!/usr/bin/env bash
# Smoke test for scripts/finalize-work-item.ts driven through its CLI with REAL git.
#
# Builds a throwaway repo layout, sets up a work item, makes a commit in its
...
FINALIZE="$SCRIPT_DIR/finalize-work-item.ts"
...
# State repo with three items waiting in todo/.
for q in "${QUEUES[@]}"; do
