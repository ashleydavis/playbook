...
#
# Builds a throwaway git-backed state repo with a tickets/ dir, then checks:
...
# Build a git-backed state repo with two ticket dirs in todo/.
STATE="$ROOT/state"
mkdir -p "$STATE/tickets/todo/demo-1" "$STATE/tickets/todo/demo-2"
echo "# demo-1" > "$STATE/tickets/todo/demo-1/index.md"
echo "# demo-2" > "$STATE/tickets/todo/demo-2/index.md"
git -C "$STATE" init -q
...
# Ticket-scoped commit: change only demo-1, commit only its path.
echo "changed" >> "$STATE/tickets/todo/demo-1/index.md"
echo "also changed" >> "$STATE/tickets/todo/demo-2/index.md"
if run_commit "touch demo-1" tickets/todo/demo-1 > /dev/null; then
    ...
    echo "$files" | grep -q "tickets/todo/demo-1/index.md" \
        || fail "commit did not touch demo-1"
    echo "$files" | grep -q "demo-2" \
        && fail "commit unexpectedly touched demo-2 (not ticket-scoped)"
else
...
out="$(run_commit "touch demo-1 again" tickets/todo/demo-1 2>&1)"
rc=$?
...
NONGIT="$ROOT/nongit"
mkdir -p "$NONGIT/tickets/todo"
echo x > "$NONGIT/tickets/todo/file.md"
out="$( ( cd "$NONGIT" && bun "$COMMIT" "msg" tickets/todo ) 2>&1 )"
rc=$?
