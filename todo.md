# Todo

- Be cool if there are different interviews for different types of projects.
- Be cool to set the CC status bar to show an overview of current stashouldfte.
- I still need planning!
    - So official support for new plans, done plans. Each plan should be a sequence of tickets that can be scheduled at a later date.
    - The thing is to check how much of the plan is still relevant given that code and docs might have changed since the plan was created.
    - I really just need to use my existing plan skills, but need a new skill to convert a plan to tickets.
    - Just need to know how an existing plan is converted to tickets. 
- Agent review doesn't need to run compile or automated tests if there are no code or configuraton 
changes.
- Feel like I need multiple Claude's open:
    - Planning
    - Review
    - Work
    - If I do this will they corrupt each others state?
- Be good to have a hold queue to keep stuff tmp
- Is it useful to maintain `current-state.md`?
    - I haven't read it once.
    - It takes Claude effort and time to keep this doc up to date.
    - If I could emulate it with bp:status maybe we don't need the file?
- Would an mcp server be a better way to manipulate the current state?
    - Most of the actions (e.g. move from todo/ to in-progress/) could be implemented as mcp tools.
    - If I want to try this, I think this would be an alternative version of Playbook so that I don't message up the original.