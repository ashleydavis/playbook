---
name: pb:add
description: Invoke when you want to queue a single, already-understood piece of work without planning a whole feature. Collects the item's details from the developer and writes a structured work item into todo/. Use for a known fix, chore, or small change. Keywords: add work item, queue a task, new work item, file a ticket, todo, known fix, chore, quick task, add to queue.
---

# pb:add

Create one structured work item in `todo/`. For a single, well-understood task. If the root cause of a bug is unknown, use `pb:debug` instead.
...
```
Developer: Add a fix item to make the login button disabled while the request is in flight.
