...
8. Create each ticket in `state/tickets/<queue>/<id>/` from `templates/ticket-template/`:
   - `index.md`: ID, Type, Depends on (omit the line if none), Failures `0`, Priority, Platforms (omit the line for `any`), and a one-line description.
...
10. Report the tickets queued, the queue they landed in, the dependency graph, and any unresolved question items left in the source.
