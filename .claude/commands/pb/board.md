---
name: pb:board
description: "Invoke for a quick, bare listing of every ticket queue and what sits in each, without the narrative or recommendation that pb:status gives. Lists the IDs (and one-line descriptions) in todo, backlog, in-progress, agent-review, human-review, merge-queue, conflicts, blocked, and the most recent in done. Use when you just want to see the board. Keywords: board, list queues, ls queues, show the board, queue contents, what's in the queue, list tickets, kanban, quick list, where are the tickets, conflicts."
---

# pb:board

A quick read of the board: which ticket sits in which queue, right now. Lighter than `pb:status`, which summarises the live queues and recommends a next skill. `pb:board` just lists the queues.
...
1. From the state repo, run the board script: `(cd state && bun ../scripts/board-tickets.ts)`. It prints JSON keyed by queue, in board order (`todo`, `backlog`, `in-progress`, `agent-review`, `human-review`, `merge-queue`, `conflicts`, then the side pen `blocked`, then `done` most-recent-first). Aborted tickets are a dead end and are deliberately left off the board. Do not list the queues by hand; do not read `detail.md`.
...
4. Do not summarise, recommend a next skill, or flag what needs the developer: that is `pb:status`.
