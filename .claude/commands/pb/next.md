...
- **Stop on a systemic failure.** If two or more items fail on the same stage or check in one run, treat it as the environment, not the items (e.g. shared test fixtures, a contended resource): reconcile every failed item first (record and route each, per the invariant above and Step 3), then stop launching new work and hand back. Handing back never means leaving items mid-stage. This is the case the old per-item timeout rule missed.
