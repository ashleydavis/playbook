...
- **Stop on a systemic failure.** If two or more items fail on the same stage or check in one run, treat it as the environment, not the items (e.g. shared test fixtures, a contended resource): reconcile every failed item first (record and route each, per the invariant above and Step 3), then **record the cause** as a `Run halted: systemic failure` entry in the top `⚠ Needs your action` section of `current-state.md` (the shared stage or check, the items involved, the suspected environmental cause, and the evidence path) and hand back. Recording the cause is required: the items themselves usually return to `todo/` and so leave no trace of why the run stopped, so without this entry the developer has nothing to act on. Handing back never means leaving items mid-stage. This is the case the old per-item timeout rule missed.
...
- **Tell the developer.** Record every block, systemic failure, and broken-main situation in the top `⚠ Needs your action` section of `current-state.md`, naming the item and the one-line reason. That section leads the file so it is the first thing the developer sees, directly or via `pb:status`.
...
2. Each turn, work the queues in processing order (**`merge-queue` → `agent-review` → `todo` → `in-progress`**; see Processing order above). **Begin every step by re-running `bun ../scripts/next-items.ts`**, because an earlier step may have moved items into this step's queue (the todo step feeds `in-progress`). It is cheap and deterministic, so always act on a freshly generated report, never a stale one:
   ...
   1. **Run the report, then process its `merge-queue` list first.** For each ID, spawn a sub-agent with:
      ...
      ```
      /goal <id> is merged into main and its directory is in done/, with every post-merge check (compile, lint, unit, smoke, e2e) passing on main and its output captured to evidence/merge/. Or stop after 15 turns.
      ```
...
3. After each turn, **reconcile first, then record state.** This step runs every turn, including when the turn ended on a systemic-failure handback.
   ...
   2. **Record state.** Now update `current-state.md` to reflect the items that moved or were created this turn (not only the abort cases but the normal forward progress, plus any Fix item spawned from a proven Debug, and every item the reconciliation just routed): add or amend only the entries these changes affect, leaving the rest of its existing content intact. Keep the file's shape: anything needing the developer (systemic failures, items in `human-review/`, blocked items) goes in the top `⚠ Needs your action` section, one or two plain lines each; routine forward progress goes in the `Progress` section below. If the turn ended on a systemic-failure handback, write the `Run halted: systemic failure` entry at the top (the shared stage or check, the items involved, the suspected environmental cause, and the evidence path), since those items usually returned to `todo/` and would otherwise leave no record of why the run stopped.
