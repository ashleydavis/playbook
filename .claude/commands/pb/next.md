...
**No shared state file.** A sub-agent's only state writes are to its own ticket: it writes its `evidence/` and adds a History note to its `detail.md` **before** moving its directory via `move.ts` (or `fail-ticket.ts`/`reset-failures.ts` on a failure path), whose ticket-scoped commit captures the evidence and History along with the transition. So the sub-agent never commits the state repo by hand. The parent never maintains any shared state file; it reports what moved in chat after the turn.
...
- **Always write ticket IDs in full** (`performance-tabs-4`, not `-4` or `pt-4`) in the run report. A leading-hyphen shorthand reads as a negative number; never abbreviate, truncate, or drop the feature prefix from an ID.
...
- **Tell the developer.** Surface every block, environmental failure, and broken-main situation in chat in the run report, naming the ticket and the one-line reason. `/pb:status` regenerates the same picture from the live queues on demand.

The one exception is a **broken main**: a merge train lands only after its checks pass on the train (which becomes main verbatim on the fast-forward), so a clean run never breaks main. If a later check on main fails anyway (a flaky or environmental check), the landed tickets are already merged and stay **done** (a merge cannot be undone). Record the broken main, surface it in chat in the run report, and stop the run because every later ticket builds on main; fixing main is new work (a fresh ticket), not a reopening of the done tickets.
...
- **Leave the queues as the record.** The queues are the durable state, so there is nothing to clean up. State in chat that the run was interrupted and to resume with `pb:next`; the queues themselves are the durable record and `pb:status` will show the in-flight state.
...
- **UI screenshots.** Any change that affects the UI must be captured as a screenshot of **every** affected view in **both light and dark mode** (every UI screenshot exists in both modes). **Screenshots are captured from the running app in its real shell** (top navbar, left sidebar, and surrounding chrome present), reaching the view the way a user would; a component rendered in isolation (a gallery, scratch render, or story) is **never** an acceptable screenshot when a page already renders the change in the running app. A single representative page is not enough: if the change is applied to several pages (e.g. pods, nodes, deployments, statefulsets, daemonsets), capture each one, in both modes. This includes a **removal**: deleting or hiding a view, page, component, or nav entry is a UI change, and its affected state is the *resulting* UI (e.g. the navigation without the removed entry, the surviving pages still working), capture that after-state. "Nothing to screenshot because it was deleted" is never a valid reason to skip them; a UI-affecting change with no screenshots is rejected at agent review. **A new or changed UI component is screenshot-required even before any page consumes it.** Adding or editing a reusable UI component (a React component, a widget, a cell/badge/card, anything that renders) is a UI change in its own right: if — and only if — no page renders it yet (its consumers land in later tickets), it must be **rendered in isolation** (a scratch render or story mounting it with sample props, covering each distinct state it carries, including any null/empty case) and screenshotted in both modes; the moment a page does render it, its screenshots must come from the running app, not from continued isolation rendering. "No page consumes it yet", "it's not wired in", "nothing to render", or "the e2e ticket will screenshot it later" are **never** valid reasons to skip a component's screenshots, and they are the only grounds for capturing in isolation rather than in the running app; a UI component committed without them is rejected at agent review exactly as an affected view is. **Every screenshot lives in a `screenshots/` subdirectory** of the pass's evidence dir (e.g. `evidence/implementation-N/screenshots/`); never put a screenshot loose in the evidence dir. Capture them however works, but never by committing capture code to `project/`: the capture script lives in the ticket's `evidence/` or a temp dir and is discarded (see Agent cleanup).
...
2. Each turn, work the queues in processing order (**`merge-queue` → `agent-review` → `todo` → `in-progress`**; see Processing order above). **Begin every step by re-running `bun ../scripts/next-tickets.ts`**, because an earlier step may have moved tickets into this step's queue (the todo step feeds `in-progress`). It is cheap and deterministic, so always act on a freshly generated report, never a stale one:
   ...
   2. **Run the report, then for each ID in its `agent-review` list,** spawn a sub-agent with these ticket completion criteria:
      ...
      **The review rejects the ticket if any of these is true:**
      ...
      - A change touches the UI but the required UI screenshots are missing (see Verification and Evidence). This includes a **new or changed UI component that ships without a screenshot of it rendered** (in isolation, with sample props, if no page consumes it yet): "not wired into a page", "no consumer yet", or "the e2e ticket covers it later" do not excuse the missing screenshots.
      - The UI screenshots were captured from a component **rendered in isolation** (a gallery, scratch render, or story) even though a page already renders the change in the running app. Screenshots of an in-app change must come from the running app in its real shell (navbar, sidebar, surrounding chrome present); isolation rendering is acceptable only for a component no page consumes yet.

      When the review finds a new fault, it records that fault as a new unticked checkbox in the ticket's `## Issues` section (in addition to a History note) before rejecting, so the issue is tracked in the section, not only in history. Resolved issues stay in the section ticked as a permanent record; checkboxes are never deleted. On a clean pass it moves the ticket to `human-review/`; on any fault it rejects the ticket back to `todo/` as a failure (routed per **When anything fails**), never fixing the work it judges. Its only writes are to its own ticket; the parent reports the outcome in chat after the turn (step 3). Problem (timeout, exhausted budget): the parent handles it the same way, recording it and routing the ticket by its count, then continues with the rest.
...
3. After each turn, **reconcile first, then report.** This step runs every turn, including when the turn ended on an environmental-failure handback.
   ...
   3. **Report.** Print the run report in chat: the tickets that moved or were created this turn (not only the abort cases but the normal forward progress, plus any Fix ticket spawned from a proven Debug, and every ticket the reconciliation just routed). Surface anything needing the developer (environmental failures, tickets in `human-review/`, blocked tickets) prominently, naming the ticket and the one-line reason; routine forward progress can be terser. If the turn ended on an environmental-failure handback, say so (the shared stage or check, the tickets involved, the suspected cause, and the evidence path), since those tickets usually returned to `todo/` and the chat report is the only record of why the run stopped. The report is transient and chat-only: the ticket-scoped commits from `move.ts`/`setup-ticket.ts`/`fail-ticket.ts` already captured every queue change, so there is no shared state file to write. The queues are the durable record and `/pb:status` regenerates the picture on demand.
...
search-4 failed agent-review (committed a change unrelated to the ticket); reviewer recorded a History note and returned it to todo/; the parent reported it and the loop continued.
