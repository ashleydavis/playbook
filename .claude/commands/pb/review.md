...
**Everything in a review is inspected against the ticket's worktree (`project/worktrees/<id>`), and nowhere else.** The app you launch, the tests you run, the diff you show, the files you read to answer a question: all of it is the code in that worktree, which is the only place the ticket's unmerged changes exist. `project/` is `main` and does **not** contain them, so it is never a valid substitute for any part of a review. This is the one rule the whole gate rests on; every option below is an instance of it, not a separate decision. If you ever cannot inspect against the worktree (it is missing, or its HEAD does not match the ticket's commit), you **stop and say so** — you never review `project/` or any other tree in its place.
...
2. **Run the inspect loop.** The verbatim card you pasted in step 1 already ends with the tailored inspect menu, so it is on screen; do not retype it here. Let the developer pick options **in any order, one at a time**. Do not dump everything at once: run the picked option, then reprint the menu and wait. The full menu, before tailoring, is:
   ...
   **Preflight, mandatory before any run / launch / test / diff (options 2, 3, 4, 7, 8).** A ticket in `human-review/` is **unmerged**: its code lives only in its worktree at `project/worktrees/<id>` (branch `worktrees/<id>`), never in `project/`, which is `main` and does **not** contain the ticket's changes. The card names this worktree on a `Worktree (run / launch / diff from HERE, never project/):` line. Before you run, launch, test, or diff anything for this ticket you MUST:
   1. Use the card's worktree path as the working directory / `git -C` target. **Never launch, test, or `bun run` in `project/`** to inspect a ticket, and never take "from the repo root" in the testing manual to mean `project/` here: the repo root that matters is the ticket's worktree.
   2. Confirm it is the code under review: run `git -C project/worktrees/<id> rev-parse HEAD` and check it equals the card's `Commit`. The card already does this check and prints a `** WARNING **` line if the worktree is missing or its HEAD does not match.
   3. If the worktree is missing, its HEAD does not match the commit, or the card printed that warning: **STOP. Say so plainly and do not run, launch, test, or diff anything.** Do **not** fall back to `project/` or any other tree. A silent fall-back to `main` shows the developer the unfixed code and is exactly the failure this gate exists to prevent. Launching an app to prove a fix works while the running code does not contain the fix is never acceptable; the running code must be proven to be the ticket's code first.
   4. **Every command you run for these options must pin the directory in the command itself, as a subshell: `(cd project/worktrees/<id> && <command>)`.** Never issue a bare `bun start`/`bun run dev`/`bun test`/setup command and rely on the session's current directory: the session cwd is `state/` (or wherever you were launched), not the worktree, so a bare command runs from the wrong tree even when you "meant" the worktree. Do not describe a command as "rooted at the worktree" unless the `cd` is literally in the command string you are about to run: **the description you print must match the command you actually issue, character for character on the directory.** Saying you added the `cd` while running a bare command is the same wrong-tree failure wearing a correct label, and it is worse than the bare mistake because it reads as fixed.
   5. **Announce the location to the developer before you run anything.** State where the work lives and commit to it out loud first: print the worktree path and, for a launch, the exact line **`I will run the app from <path>`** (for tests, `I will run the tests from <path>`), where `<path>` is the ticket's worktree. Say it before the command, not after. This is a checkpoint for the developer: naming the path up front lets them catch a wrong one before it runs, instead of discovering it by clicking a broken app. The `<path>` you announce and the directory in the command you then run must be the same worktree.
   ...
   **Showing means pasting into your reply (mandatory, applies to every "show" pick and to any ad-hoc "show me ..." the developer types).** When the task is to *show* content (a file, a diff, a doc, command output), the content must appear **in the body of your reply as a fenced code block** (this is for text content; screenshots are not rendered inline, see option 1 below). Running `git show`/`git diff`/`cat`/`Read`/a test command does **not** count as showing it: command output and terminal scrollback are not your message and the developer may never see them. If you ran a command to fetch the content, copy the relevant content into your reply. For brand-new files, paste the file content itself, not a `+`-prefixed diff with repeated commit headers. Output the content and stop: add no description, recap, analysis, verdict, or summary after it (see **Show only, nothing trailing** in the Output style above). If the developer says they did not see what they asked for, assume it went to command output and re-send it inline.
   ...
   1. **Show the screenshots.** List the PNG files in the card's **screenshots directory** (the card gives its path and count) to get their paths. State each screenshot's **full file path on disk** (e.g. `state/tickets/human-review/<id>/evidence/implementation-N/screenshots/<name>.png`), then **open them for the developer** in their image viewer (`xdg-open <path>` on Linux, `open <path>` on macOS), one command per screenshot. List every path. Showing the paths is mandatory, not optional. **Do not render the screenshots inline.** A `Read` of a PNG produces a tool result, which is scrollback the developer never sees in chat, so it is wasted work; `xdg-open` is the only thing that reaches their viewer. **Once the screenshots are open, this option is done: return to the inspect menu immediately.** Do not read other files, and add no description, analysis, or verdict.
      ...
      **Never ask the developer how they want the screenshots shown, and never ask which ones to open. Open every screenshot in the pass, however many there are.** Picking this option is already the answer; there is no decision left to make. The count is not a decision point: 6 or 64, open them all. Do not offer a subset, a "representative sample", light-mode-only, the folder instead of the files, or a menu of any of these. Do not stop to flag that the count is large. The developer asked to see the evidence; making them choose how hands your job back to them and stalls the review at the exact moment they are ready to look. If a count is genuinely unwieldy, that is a defect in the capture to raise elsewhere, not a question to put at this gate.
   2. **Run it by hand.** Do the Preflight above first. Work out which part of the app this ticket changed (from its card's changed-files and `detail.md`), then give the developer everything they need to run and explore it themselves **from the ticket's worktree** (`project/worktrees/<id>`), not `project/`. Pull the exact commands from the **testing manual** (`project/docs/testing-manual/`) if the project keeps one, else from the README or run docs; only if none exist, work them out from the code. Do not invent commands the project already documents. When the manual says "from the repo root", root the command at the worktree instead, as a subshell: `(cd project/worktrees/<id> && bun run dev)`. Cover all six, in order:
      ...
      2. **Setup commands.** The exact command(s) from the manual to set up (e.g. load database fixtures, start test clusters), rooted at the worktree.
      3. **Start command(s).** The exact command(s) from the manual to start the app, rooted at the worktree.
   ...
   3. **Start it for you.** Do the Preflight above first, and stop if it fails. **Announce the location before launching**: print the worktree path and the line `I will run the app from <path>` (see Preflight item 5), so the developer can catch a wrong path before it runs. Then launch the app with the start command as a subshell rooted at that worktree: `(cd project/worktrees/<id> && bun run dev)` (or the manual's start command). The `cd` must be literally inside the command you run; never a bare `bun run dev` relying on the session cwd. **Then verify the running app is actually the worktree's before you hand it over**: confirm the serving process is rooted under `project/worktrees/<id>` (e.g. read `/proc/<pid>/cwd` for the process on the app's port, or check the launch log shows the worktree path), not `state/`, `project/`, or your session cwd. If it is not rooted in the worktree, **kill it and stop**; do not hand the developer an app you have not confirmed is the ticket's code. Only once verified, tell them what to look at in the app for this change (which view, what they should see). **Do not drive or navigate the app yourself** (no clicking, typing, or routing): just start it and hand it over. The developer explores it and closes it themselves when they are ready.
   4. **Run the automated tests.** Do the Preflight above first. Say which tests you will run (the card's Test Plan / test results name which apply), then run them fresh in the foreground as a subshell rooted at the worktree, `(cd project/worktrees/<id> && <test command>)`, never `project/` and never a bare command (unit, smoke, and/or e2e, picking the levels that match what changed), and show the output.
   ...
   8. **View the code diff yourself.** Do not show the diff: give the developer the exact command(s) to find and view it themselves (e.g. `git -C project/worktrees/<id> show <commit>`, or `git -C project show <commit>` since the commit is reachable from the project repo too), naming the files changed so they know what to look at.
...
- **Abort:** to `aborted/`. The developer is killing the ticket: the work is abandoned and will not be done. A reason note is **optional**; if they give one, append it to the ticket's History section as the abort reason before the move. Then move the directory to `aborted/`, which sets the ticket's state to aborted (the queue it sits in is its status). Unlike a rejection, an aborted ticket does not rejoin the loop and its `**Failures:**` count is left untouched; `pb:next` never touches `aborted/`. The `aborted/` directory is its only record. **Because the work is abandoned, always tear down its in-flight worktree and branch** (unlike block/backlog, this is not optional: an aborted ticket will never be re-implemented, so leaving the worktree and branch behind just orphans them). Remove the worktree and delete the branch: `git -C project worktree remove --force project/worktrees/<id>` then `git -C project branch -D worktrees/<id>` (if the worktree is already gone, run `git -C project worktree prune` first, then delete the branch). Do this before the move to `aborted/`.
