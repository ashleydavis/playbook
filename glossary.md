...
- **Review snapshot**: the temporary, git-ignored JSON file that drives a `pb:review` session (`state/.pb-review-snapshot.json`). Created by `start-review.ts` as a snapshot of `human-review/` when review starts, it is the source of truth for the review checklist: the rows, their fixed order and numbers, each ticket's precomputed render card, and an `updatedAt` stamp. `format-ticket-selection.ts` renders and marks it; a stale snapshot is rebuilt from the live queue. Refer to it as "the review snapshot", not "the JSON file".
