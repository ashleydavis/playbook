...
Sets or clears `**Platforms:**` on tickets in `todo/` or `backlog/`. A ticket locked to platforms is admitted to `in-progress/` by `/pb:next` only on a machine running one of them; `any` clears the lock. Platforms can also be set when a ticket is created. See [.claude/commands/pb/platforms.md](.claude/commands/pb/platforms.md).
...
Says what `/pb:next` would do if run now, without doing any of it. It runs the same report `/pb:next` acts on and describes, queue by queue in the same processing order, which tickets would be rebased, merged, reviewed, admitted and implemented. It changes nothing: no moves, no worktrees, no sub-agents, no commits. It covers the first turn only, since later turns depend on how each ticket fares. See [.claude/commands/pb/next/preview.md](.claude/commands/pb/next/preview.md).
...
**Opening on a selected ticket.** As soon as a ticket is selected, `/pb:review` prints a list of the ticket's **prior human rejections** (oldest first, each with a very brief note on how it was subsequently rectified, or "none"), then the ticket's **summary card**: its name, description, changed-file and screenshot **counts** (not the full lists), latest evidence pass, test results, commit, and its tailored inspect menu. The human-rejection list comes from the ticket's History; the rest is the card. The card is deliberately short so the developer is not buried in a wall of paths on selection; the full detail (every changed file, every screenshot) is one inspect-menu pick away, opened on demand. The inspect menu at the end of the card is the action prompt.
...
An agent allocates its subdir by taking one more than the highest existing number for its kind (the first implement pass writes `implementation-1/`, the next `implementation-2/`, and likewise for `review-N/`); the merge stage writes to `evidence/merge/`.

Older passes are kept only as history: nothing ever reaches back into them to fill a gap in a newer pass. The **latest** `implementation-N/` must, on its own, hold **all** the evidence needed to prove the **entire** change (not just the part that pass altered), because human review and the ticket card read only the latest pass, so a fact missing from it is treated as missing from the ticket. And every artifact must be **captured fresh from the code in that pass's commit**: screenshots and other evidence are never copied, carried forward, or reused from an earlier `implementation-N/`, since evidence from a previous version of the code no longer proves the current code. A re-implementation re-captures the whole required set into its own subdir even when its diff is one line or touches no UI, and agent review rejects a latest pass whose evidence is incomplete or was not freshly captured. What goes in each subdir:
...
- **Screenshots.** For any change that affects the UI, a screenshot of each affected view in **both light and dark mode** (every UI screenshot exists in both modes), in a `screenshots/` subdirectory. Capture them however works, but never by committing capture code to `project/`: a screenshot must need no committed change to the project tree (no added test, helper, or env plumbing). Any capture script lives outside the project tree, in the ticket's `evidence/` dir or a temp dir, and is discarded. Every screenshot the latest pass needs must sit in that pass's own `screenshots/` dir, **captured fresh from that pass's code**; a re-implementation re-captures them all against its own commit and never copies or carries them forward from an earlier `implementation-N/`, even when the pass alters no pixel.
